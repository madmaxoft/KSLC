...
{$ELSE}
  R := FindFirstUTF8(Path, Attr, SearchRec);
{$ENDIF}
  Result := R = 0;
  if Result then
  begin
...
{$ELSE}
            PartialResult := DeleteFileUTF8(FileName);
{$ENDIF}
        end;
      end;
...
{$ELSE}
    if FindFirstUTF8(FileName, faAnyFile, SearchRec) = 0 then
{$ENDIF}
    begin
...
{$ELSE}
  Result := ForceDirectories(ExtractFilePath(Name)) and CreateDirUTF8(Name);
{$ENDIF}
end;
...
{$ELSE}
  if FindFirstUTF8(FileName, faAnyFile, Result) = 0 then
{$ENDIF}
...
{$ELSE}
      Rslt := FindFirstUTF8(Folders[CurrentItem] + '*.*', faDirectory, FindInfo);
{$ENDIF}
      try
        while Rslt = 0 do
        begin
...
{$ELSE}
    Rslt := FindFirstUTF8(CurrentFolder + FileMask, LocAttr, FindInfo);
{$ENDIF}
    try
      while Rslt = 0 do
      begin
