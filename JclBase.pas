...
// IsBadReadPtr() causes an AV on invalid addresses, we try to minimize that for known bad addresses
function JclIsBadReadPtr(iAddr: pointer; iBytes: cardinal): LONGBOOL;
...
function JclIsBadReadPtr(iAddr: pointer; iBytes: cardinal): LONGBOOL;
begin
	// $30000 seems to be a very common value and causes needless exceptions in IsBadReadPtr(), so we check it explicitly
	if (iAddr = pointer($030000)) then
	begin
		OutputDebugString('Caught a 0x030000 bad read ptr query'#10);
		Result := TRUE;
		Exit;
	end;

	Result := IsBadReadPtr(iAddr, iBytes);
end;
