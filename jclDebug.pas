...
type
  ...
  public
		...
		procedure AddToStrings(const Strings: TStrings; IncludeModuleName: Boolean {$IFDEF SUPPORTS_DEFAULTPARAMS} = False {$ENDIF}; IncludeParams: Boolean {$IFDEF SUPPORTS_DEFAULTPARAMS} = False {$ENDIF});
...
function IntToTag4(i: cardinal): string;
begin
	Result := '';
	if ((i shr 24) > 31) then
		Result := Result + chr((i shr 24) and $ff)
	else
		Result := Result + ' ';

	if (((i shr 16) and $ff) > 31) then
		Result := Result + chr((i shr 16) and $ff)
	else
		Result := Result + ' ';

	if (((i shr 8) and $ff) > 31) then
		Result := Result + chr((i shr 8) and $ff)
	else
		Result := Result + ' ';
...
{$ELSE}
  if FileExistsUTF8(MapFileName) then
{$ENDIF}
...
const
  TableHeader               = 'Start         Length     Name                   Class';
	...
	PublicsByValueHeaderMSVC6 = 'Address         Publics by Value              Rva+Base     Lib:Object';
var
  ...
  P1, P2: PJclMapString;
	...
	begin
		while not(Eof()) and (CurrPos^ in AnsiWhiteSpace) do
			Inc(CurrPos);
	end;
...
begin
  if FStream <> nil then
    ...
    PreviousA.Offset := 0;
		...
		begin
			// _X: we probably have a MSVC6 MAP file without the detailed map, use the class map instead:
			CurrPos := FStream.Memory;
			if SyncToHeader(TableHeader) then
			begin
				while IsDecDigit do
		...
		// _X: check for MSVC 6 map file data:
		CurrPos := FStream.Memory;
		if (SyncToHeader(PublicsByValueHeaderMSVC6)) then
		begin
			while IsDecDigit do
			begin
				A := ReadAddress();
...
var
  ...
  Sections, LastSection, JclDebugSection: Windows.PImageSectionHeader;
...
{$ELSE}
    DebugInfoList := TJclDebugInfoList.Create(False);
{$ENDIF}
end;
...
{$ELSE}
  Result := FileExistsUTF8(MapFileName);
{$ENDIF}
  if Result then
...
{$ELSE}
    Result := FileExistsUTF8(JdbgFileName);
{$ENDIF}
    if Result then
    begin
...
function ModuleFromAddr(const Addr: Pointer): HMODULE;
var
  MI: Windows.TMemoryBasicInformation;
begin
...
begin
  ...
  if IncludeModuleName then
    Insert(Format('{%-18s}', [ExtractFileName(GetModulePath(ModuleFromAddr(Addr)))]), Result, 11);
end;
...
begin
  ...
  Result := (CodeAddr > 8) and ValidCodeAddr(DWORD(C8P)) and
    not JclIsBadReadPtr(C8P, 8) and not IsBadCodePtr(C8P) and not IsBadCodePtr(C4P);
...
{$ELSE}
    List.AddToStrings(Strings, IncludeModuleName, False);
{$ENDIF}
end;
...
begin
  ...
  BaseOfStack := DWORD(StackFrame) - 1;
	...
	NumItems := 0;
	while ((NextStackFrame(StackFrame, StackInfo)) and (NumItems < 1000)) do
	begin
...
begin
	...
	NumItems := 0;
	while ((DWORD(StackPtr) < TopOfStack) and not(JclIsBadReadPtr(StackPtr, 8)) and (NumItems < 1000)) do
	begin
...
{$R-}
procedure TJclStackInfoList.AddToStrings(const Strings: TStrings; IncludeModuleName: Boolean; IncludeParams: Boolean);
var
	I, j: Integer;
	txt: string;
	IsSuccess: boolean;
begin
	...
	begin
		...
		begin
			...
			// DEBUG:
			OutputDebugString(PChar(Format('Processing arguments for callsite %x [%d of %d], at ptr %x'#10, [integer(Items[I].StackInfo.CallerAdr), i, Count, integer(Items[I].StackInfo.DumpPtr)])));

			for j := (Items[I].StackInfo.DumpSize div 4) - 1 downto 0 do
			begin
				txt := #9;
				try
					if Assigned(Items[i]) then
					begin
						if Assigned(Items[i].StackInfo.DumpPtr) then
						begin
							if (not(JclIsBadReadPtr(Items[I].StackInfo.DumpPtr, j * sizeof(integer)))) then
							begin
								txt := txt + IntToHex(PIntegerArray(Items[I].StackInfo.DumpPtr)^[j], 8) + #9;
								txt := txt + '"' + IntToTag4(PIntegerArray(Items[I].StackInfo.DumpPtr)^[j]) + '"'#9;
								txt := txt + GetLocationInfoStr(PPointerArray(Items[I].StackInfo.DumpPtr)^[j], IncludeModuleName);
							end
							else
							begin
								txt := txt + '(Items[i].StackInfo.DumpPtr^[' + IntToStr(j) + '] (0x' + IntToHex(integer(Items[I].StackInfo.DumpPtr) + j * sizeof(integer), 8) + ') not accessible'#9;
								IsSuccess := false;
							end;
						end
						else
						begin
							txt := txt + '(Items[i].StackInfo.DumpPtr = nil'#9;
							IsSuccess := false;
						end;
					end
					else
					begin
						txt := txt + '(Items[i] = nil'#9;
						IsSuccess := false;
					end
				except
					txt := txt + ' (Exception while evaluating!)';
					IsSuccess := false;
				end;
				Strings.Add(txt);
...
//    Result := pointer(pinteger(PJmpTable(Result))^);
    if not JclIsBadReadPtr(PJmpTable(Result).Ptr, 4) then
...
{$ELSE}
    VTable := Pointer(Integer(ExceptObj.ClassType) + vmtMsgStrPtr);
{$ENDIF}
...
{$ELSE}
        Move(PChar(VTable)[vmtParent - vmtMsgStrPtr], VTable, 4);
{$ENDIF}
        if VTable = nil then
        begin
{$IFnDEF FPC}
...
procedure TJclExceptFrameList.AddToStrings(const Strings: TStrings; IncludeModuleName: Boolean {$IFDEF SUPPORTS_DEFAULTPARAMS} = False {$ENDIF});
(*
const
	MAX_TRYLEVEL = 1000;

type
	RVCScopeTable = packed record
		PreviousTryLevel: DWORD;
		FilterPtr: pointer;
		HandlerPtr: pointer;
	end;
	PVCScopeTable = ^RVCScopeTable;
	TVCScopeTableArr = array [0..MAX_TRYLEVEL] of RVCScopeTable;
	PVCScopeTableArr = ^TVCScopeTableArr;
*)
...
var
	...
	(*
	Frame: PExcFrame;
	TryLevel, NewTryLevel: DWORD;
	VCFrames: TStrings;
	VCFramesValid: boolean;
	ScopeTable: PVCScopeTableArr;
	*)
begin
	for i := 0 to Count - 1 do
	begin
		Strings.Add(GetLocationInfoStr(Pointer(Items[i].CodeLocation()), IncludeModuleName));

		(*
		// _X 2011_05_08: Found out that VC++ frames aren't handled the same in VS2008 anymore. Disabling frame detection
		// There's never been any useful info out of it anyway.
		Frame := Items[i].ExcFrame;
		if (MyIsBadReadPtr(Frame, 20)) then
		begin
			continue;
		end;
		// VC_Vars may be accessed safely. Check ScopeTable accessibility:
		VCFrames := TStringList.Create();
		try
			TryLevel := Frame.VC_TryLevel;
			if (TryLevel < MAX_TRYLEVEL) then
			begin
				VCFramesValid := true;
				ScopeTable := PVCScopeTableArr(integer(Frame.VC_ScopeTable));
				while not(MyIsBadReadPtr(@(PVCScopeTableArr(ScopeTable)[TryLevel]), sizeof(RVCScopeTable))) do
				begin
					if (TryLevel >= MAX_TRYLEVEL) then
					begin
						// The last TryLevel should be $ffffffff, check:
						VCFramesValid := (TryLevel and $ffffff00 = $ffffff00);  // VC2008 uses 0xfffffffe instead of 0xffffffff
						break;
					end;
					VCFrames.Add(Format('VC frame found, trylevel = %d', [TryLevel]));
					VCFrames.Add(#9'Filter  = ' + GetLocationInfoStr(ScopeTable[TryLevel].FilterPtr, IncludeModuleName));
					VCFrames.Add(#9'Handler = ' + GetLocationInfoStr(ScopeTable[TryLevel].HandlerPtr, IncludeModuleName));
					NewTryLevel := ScopeTable[TryLevel].PreviousTryLevel;
					if (NewTryLevel > TryLevel) then
					begin
						VCFramesValid := false;
						break;
					end;
					TryLevel := NewTryLevel;
				end;  // while not(MyIsBadReadPtr())
				if (VCFramesValid) then
				begin
					Strings.AddStrings(VCFrames);
				end;
			end;
		finally
			VCFrames.Free();
