...
{******************************************************************************}
{                                                                              }
{ Various RunTime Type Information routines. Includes retrieving RTTI          }
{ information for different types, declaring/generating new types, data        }
...
type
  IJclInfoWriter = interface
    ...
    procedure Writeln(const S: string {$IFDEF SUPPORTS_DEFAULTPARAMS} = '' {$ENDIF});
    procedure Indent;
    procedure Outdent;
  ...
  public
    constructor Create(const AWrap: Integer {$IFDEF SUPPORTS_DEFAULTPARAMS} = 80 {$ENDIF});
  ...
  public
    constructor Create(const AStrings: TStrings;
      const AWrap: Integer {$IFDEF SUPPORTS_DEFAULTPARAMS} = 80 {$ENDIF});
...
type
  TJclIsFunc = function(const AnObj: TObject; const AClass: TClass): Boolean;
...
var
  ...
  OrgAsClass: Pointer;
  IsHooked: Boolean;
  SavedIs: array [0..4] of Byte;
  SavedAs: array [0..4] of Byte;
...
var
  OldProt: DWORD;
begin
  if not JclIsHooked then
  begin
    if VirtualProtect(OrgIsClass, SizeOf(TJmp), PAGE_READWRITE, OldProt) then
      try
        PJmp(OrgIsClass).OpCodeJmp := $E9;
        PJmp(OrgIsClass).JmpOffset := Longword(Addr(IsProc)) -
          ((Longword(OrgIsClass) + SizeOf(TJmp)-1));
      finally
        VirtualProtect(OrgIsClass, SizeOf(TJmp), OldProt, OldProt);
      end;
    if VirtualProtect(OrgAsClass, SizeOf(TJmp), PAGE_READWRITE, OldProt) then
      try
        PJmp(OrgAsClass).OpCodeJmp := $E9;
        PJmp(OrgAsClass).JmpOffset := Longword(Addr(JclAsClass)) -
          ((Longword(OrgAsClass) + SizeOf(TJmp)-1));
      finally
        VirtualProtect(OrgAsClass, SizeOf(TJmp), OldProt, OldProt);
      end;
    IsHooked := True;
    Result := True;
  end
...
begin
  ...
  if Result then
  begin
    if VirtualProtect(OrgIsClass, SizeOf(TJmp), PAGE_EXECUTE_READWRITE,
        OldProt) then
      try
        Move(SavedIs, PChar(OrgIsClass), SizeOf(SavedIs));
      finally
        VirtualProtect(OrgIsClass, SizeOf(TJmp), OldProt, OldProt);
      end;
    if VirtualProtect(OrgAsClass, SizeOf(TJmp), PAGE_EXECUTE_READWRITE,
        OldProt) then
      try
        Move(SavedAs, PChar(OrgAsClass), SizeOf(SavedAs));
      finally
        VirtualProtect(OrgAsClass, SizeOf(TJmp), OldProt, OldProt);
      end;
    IsHooked := False;
