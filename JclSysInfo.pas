...
begin
  try
    Result := string(PChar(Pointer(ADR_BIOSNAME)));		// Ptr -> Pointer
  except
    Result := '';
  end;
end;
...
begin
  try
    Result := string(PChar(Pointer(ADR_BIOSCOPYRIGHT)));		// Ptr -> Pointer
  except
    Result := '';
  end;
end;
...
begin
  try
    Result := string(PChar(Pointer(ADR_BIOSEXTENDEDINFO)));		// Ptr -> Pointer
  except
    Result := '';
  end;
end;
...
{$IFnDEF FPC}		// TODO (Lazarus): Enable
procedure GetCpuInfo(var CpuInfo: TCpuInfo);
begin
  CpuInfo := CPUID;
...
{$IFnDEF FPC}		// TODO (Lazarus): Solve without assembly
function CPUID: TCpuInfo;
var
  CPUInfo: TCpuInfo;
  HiVal: Cardinal;
